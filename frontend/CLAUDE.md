...
FOUNDR must not help plan businesses whose core purpose, operating method, or central advantage is illegal, exploitative, rights-violating, or seriously harmful. The shipped `ALLOW` / `BLOCK` LLM classifier uses an 18-category behavioral taxonomy and reasons across purpose, customer/target, consent, mechanism, authorization, safeguards, and foreseeable effects—not keywords. It is enforced at three points: before tailored-question generation, before session persistence using the combined questionnaire, and before a fresh arena starts. A block shows fixed safe refusal copy and never generates questions, creates a session, or starts an advisor turn. Classifier failure is retryable and fails closed. The non-authenticated `/guardrail-lab` route is development-only, non-persisting, and runs the exact classifier against 46 paired/adversarial cases. Canonical policy: [`.claude/GUARDRAIL.md`](../.claude/GUARDRAIL.md).
...
- [x] Add `SAFETY_CLASSIFIER_SYSTEM` + `classifyIdea` through the shared Gemini→Groq provider layer (temperature 0, validated JSON verdict, 18 behavior-based categories, dual-use/mental-health boundaries, multilingual and prompt-injection resistance)
...
- [x] Content-safety guardrail: intake gate before question generation, combined idea+questionnaire gate before persistence, fresh-arena bypass safety-net, accessible refusal popup/card, and development-only `/guardrail-lab` intake replica with a 46-case paired/adversarial suite
...
- [x] Phase 4
- [x] Phase 5 *(code complete + DB tested; three-layer 18-category content-safety gate shipped; development-only intake lab added; final paced live suite passed 46/46 decisions and 28/28 categories with zero provider errors; live authenticated browser E2E still pending)*
...
> 1. **Browser-verify the guardrail and full War Room:** use public development-only `/guardrail-lab` to confirm arbitrary ALLOW/BLOCK results and the quick suite UI without Auth0; then in the authenticated intake confirm a harmful test idea shows the refusal popup before questions and a legitimate idea proceeds. Verify the debate keeps the roundtable beside the independently scrolling dialogue rail, round dividers are legible, both 10-second intermissions pause orchestration/skip correctly, and the responsive layout stacks cleanly. Then complete synthesis and confirm the map renders (central "The Idea" hub + spokes + non-overlapping per-agent constellations), zoom/pan work, click→panel works, and remediation visibly re-lays-out the node (legible HITL).
